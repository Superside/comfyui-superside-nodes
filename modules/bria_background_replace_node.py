...
class SupersideBriaBackgroundReplaceNode(
    ...
    This is a SEPARATE, richer endpoint from the classic "Bria Replace
    Background V2" node (bria/replace-background). It adds reference-image
    guidance for the new background, prompt refinement, a fast/quality
    toggle, and generating several variations at once. Like all these
    models it is generative - it re-lights the scene, so it is NOT a way to
    get an exact flat hex color (use "Bria Background Standardizer (Hex
    Color)" for that). Trained on licensed data for commercial use.
    """
    ...
    DESCRIPTION = (
        ...
        "(endpoint fal-ai/bria/background/replace - the richer model, separate "
        "from the classic Bria Replace Background V2). Guide it with a text "
        "prompt and/or a reference image; supports multiple variations."
    )
