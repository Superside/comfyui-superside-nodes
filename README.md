...
- **Inputs:** `image`, `api_key` · optional: one BOOLEAN per section (defaults mirror the original re-skin workflow's exclusion set: nose/eyes/ears/mouth/lips/hair/hat on, the rest off), one `<section>_opacity` FLOAT per section (default 1.0), `padding_percent`, `glasses_prompt_override`, `glasses_box_center_x/y` + `glasses_box_width/height` (optional GroundingDINO-style box for the glasses section)
