...
Prompt driver for interior / real-estate image generation, built for a LoRA trained on three interior styles: **transitional**, **traditional**, **modern**. Composes a prompt fragment from three axes - `style` × `room` × realism `level` - where style + room define the concrete look (materials, cabinetry, furniture, lighting, ceilings, floors, fixtures, palette) and level controls photographic detail (and drives `strength` / `lora_scale`). Rooms: `any`, `living_room`, `bedroom`, `kitchen`, `dining_room`, `bathroom`, `hallway`. Optional `trigger_word` is prepended verbatim (for a LoRA trained with a trigger token), and `include_base` appends the shared "warm evening real-estate photography" look all three styles share. No API key, no model call. The full human-readable vocabulary is the master glossary in [`modules/architectural_styles_glossary.txt`](modules/architectural_styles_glossary.txt); the node's condensed phrases mirror it - expand the .txt first, then mirror into the node.
- **Inputs:** `style` (transitional / traditional / modern), `room` (7 options, default `any`), `level` (5 presets, default `3 - medium`) · optional: `trigger_word` (STRING), `include_base` (BOOL, default ON)
- **Outputs:** `prompt_fragment` (STRING), `lora_scale` (FLOAT), `strength` (FLOAT)
